...
export default class Card {
  ...
  // Actualiza el conteo de likes en la tarjeta
  _updateLikes() {
    const likeCount = this._element.querySelector('.cards__like-count');
    likeCount.textContent = this._likes.length;
  ...
  // Establece los listeners en los botones de la tarjeta
  _setEventListeners() {
    this._element.querySelector('.cards__like-icon').addEventListener('click', () => this._toggleLike());
    this._element.querySelector('.cards__image').addEventListener('click', () => this._handleCardClick(this._link, this._name));
    this._element.querySelector('.cards__delete-icon').addEventListener('click', () => this._handleCardDelete());
  }

  // Método para manejar la eliminación de la tarjeta con la API
  _handleCardDelete() {
    this._openDeletePopup(this._id, () => this._removeCard());
  }
  ...
  createCard() {
    ...
    if (this._likes.some(user => user._id === this._userId)) {
      this.getDeleteButton().style.display = 'block';
    } else {
      this.getDeleteButton().style.display = 'none';
    }
  ...
  // Obtiene el botón de eliminar la tarjeta
  getDeleteButton() {
    return this._element.querySelector('.cards__delete-icon');
  }
}
